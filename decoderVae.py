...
class DecoderVAE(nn.Module):
    def __init__(self, word_embedding, src_word_size, src_context_size, KG_word_size,
                       rnn_size, num_layers, num_softmax, latent_size, dropout = 0):
        ...
        self.context_vae = VAE_gaussian.VariationalAutoEncoder(self.src_word_size + self.src_context_size, self.rnn_size, 
                                                               self.latent_size, self.latent_size)
        self.KG_vae = VAE_gaussian.VariationalAutoEncoder(self.KG_word_size, self.rnn_size,
                                                          self.latent_size, self.latent_size)
        self.rnn_cell = rnn_cell.StackedGRUCell(self.emb_size + self.KG_word_size + latent_size, 
                                                self.rnn_size,
                                                self.num_layers,
                                                self.dropout)
        self.context2hidden = nn.Linear(self.src_context_size, self.rnn_size * self.num_layers)
        self.attention = attention.Attention(self.src_word_size, rnn_size, type = 'mlp') 
        self.KG_attention = attention.Attention(self.KG_word_size, rnn_size + src_word_size, type = "mlp")
    ...
    def forward_step(self, src_context_output, src_word_output, src_word_mask, 
        ...
        """
            Args:
                ...
                context_latent (FloatTensor)     : (batch_size, context_latent_size)
                KG_latent (FloatTensor)          : (batch_size, KG_latent_size)
                combine_knowledge (Boolean)      : Whether to incoporate copy mechanism
                tgt_word_output (FloatTensor)    : (batch_size, tgt_word_len, word_size)
            Regurns:
        ...
        combined_latent = .5 * context_latent + .5 * KG_latent
        # rnn_inputs : [barch_size, emb_size + src_KG_size + latent_size]
        #  print(tgt_word_emb.size())
        #  print(attn_KG_word.size())
        #  print(combined_latent.size())
        rnn_inputs = torch.cat([tgt_word_emb, attn_KG_word, combined_latent], dim = 1)
        ...
        return logit_word, hidden

    def greedy_generate(self, src_context_output, src_word_output, src_word_len, KG_word_output, 
                        KG_word_len, KG_word_seq, init_tgt_word_input, max_tgt_word_len, combine_knowledge,
        ...
        # context_latent : [batch_size, context_latent_size]
        context_latent = self.context_vae(input = torch.cat([src_word_output.mean(dim = 0), src_context_output], dim = 1),
                                          additional_input = None, inference_mode = False)
        # KG_latent : [batch_size, KG_latent_size]
        KG_latent = self.KG_vae(input = KG_word_output.mean(dim = 0), additional_input = None, inference_mode = False)
        ...
        for word_index in range(max_tgt_word_len):
            # hidden : [num_layer, batch_size, rnn_size]
            # logit_word : [batch_size, num_vocab]
            logit_word, hidden = self.forward_step(src_context_output, src_word_output, src_word_mask,
                                                   KG_word_output, KG_word_seq, KG_word_mask, hidden, 
                                                   tgt_word, context_latent, KG_latent, combine_knowledge)
    ...
    def forward(self, src_context_output, src_word_output, src_word_len, 
                KG_word_output, KG_word_len, KG_word_seq, 
                tgt_word_input, tgt_word_output, combine_knowledge):
        ...
        # obtain latent vairables
        # context_latent : [batch_size, context_latent_size]
        context_latent, context_kld = self.context_vae(input = torch.cat([src_word_output.mean(dim = 0), src_context_output], dim = 1),
                                                       additional_input = tgt_word_output.mean(dim = 1))
        # KG_latent : [batch_size, KG_latent_size]
        KG_latent, KG_kld = self.KG_vae(input = KG_word_output.mean(dim = 0),
                                        additional_input = tgt_word_output.mean(dim = 1))
        ...
        for word_index in range(max_tgt_len):
            # recurrence
            logit_word, hidden = self.forward_step(src_context_output, src_word_output, src_word_mask,
                                                   KG_word_output, KG_word_seq, KG_word_mask,
                                                   hidden, tgt_word_input[:,word_index], 
                                                   context_latent, KG_latent, 
        ...
        return logit, context_kld, KG_kld
