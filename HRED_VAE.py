...
class HRED_VAE(nn.Module):
    def __init__(self, num_vocab, emb_size, enc_word_rnn_size, enc_word_num_layers, enc_context_rnn_size, 
                enc_context_num_layers, KG_word_rnn_size, KG_word_num_layers, dec_rnn_size, dec_num_layers,
                 dec_num_softmax, latent_size, dropout, pre_embedding = None):
        super(HRED_VAE, self).__init__()
        ...
        self.KG_encoder = WordEncoderRNN(word_embedding = self.word_embedding,
                                         ...
                                         dropout = dropout)
        self.decoder = DecoderVAE(word_embedding = self.word_embedding,
    ...
    def forward(self, src_sents, src_word_len, src_utterance_len, KG_sents, KG_word_len, 
                      tgt_word_input, tgt_word_len, combine_knowledge):
        """
        ...
        tgt_word_output,_ = self.context_encoder.word_encoder(tgt_word_input, tgt_word_len)
        # logit : [batch_size, tgt_word_len, num_vocab] 
        # converage : [batch_size, tgt_word_len]
        logit, context_kld, KG_kld = self.decoder(src_context_outputs, src_word_outputs, src_word_len, 
                                                  KG_word_output, KG_word_len, KG_sents, 
