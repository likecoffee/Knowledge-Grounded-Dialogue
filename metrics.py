...
def diversity_metrics(hypothesis, none_set):
    filter_fun = lambda x: x not in none_set
    hypothesis = [list(filter(lambda x: x not in none_set, sent))
    ...
    unfrequent_num_unigram = np.sum(np.array(unigram_list) > 1000)
    novelty = unfrequent_num_unigram / total_num_unigram
    return dist_1, dist_2, novelty

def evaluate_response(hypothesis, references, embedding_array, none_set):
    avg_cos_similarity, extreme_cos_similarity, greedy_scores = embedding_metrics(hypothesis, references, embedding_array)
    h_dist_1,h_dist_2,h_novelty = diversity_metrics(hypothesis, none_set)
